...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
		// for(int i=0; i < ARRAY_SIZE / 8 * 8; i++) {
		// 	z[i] = x[i] + y[i];
		// 	z[i + 1] = x[i + 1] + y[i + 1];
		// 	z[i + 2] = x[i + 2] + y[i + 2];
		// 	z[i + 3] = x[i + 3] + y[i + 3];
		...
		// printf("%d\n%d\n", num, index);
		// printf("%s\n", "---------------------------");
	    for (int i = index; i < ARRAY_SIZE; i += num) {
	      	z[i] = x[i] + y[i];
	    }
	}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
	#pragma omp parallel
	{
		...
		// printf("%d\n%d\n", num, index);
		// printf("%s\n", "---------------------------");
    	for(int i = index * chunk_size; i < ((index + 1) * chunk_size); i +=1) {
      		z[i] = x[i] + y[i];
    	}

		for (int i = ARRAY_SIZE / chunk_size * chunk_size; i < ARRAY_SIZE; i += 1) {
			z[i] = x[i] + y[i];
		}
